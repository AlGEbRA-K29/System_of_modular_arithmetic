...
class polynomial_ring {
	...
	[[maybe_unused]] [[nodiscard]] bigint inverseElement(const bigint& to_inverse) {
		for (bigint i = bigint(0); i < modulus; ++i) {
			if (((to_inverse % modulus) * (i % modulus)) % modulus == bigint(1)) {
				return i;
			}
		}
...
public:
	...
	void normalize() {
		...
		if (highest_degree == data.rend() || highest_degree->second == bigint(0)) {
			return;
		}
	...
	polynomial_ring quickPow(bigint power) const {
		if (power < bigint(1)) {
			throw std::invalid_argument("Power value must be greater than 0");
		}

		polynomial_ring result({bigint(1)}, modulus);
		polynomial_ring multiplier = *this;

		while (!power.isZero()) {
			if (power % 2_BI == bigint(0)) {
